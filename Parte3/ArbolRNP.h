...
class ArbolRNP {
	friend ostream& operator<<(ostream& salida, ArbolRNP &arbol){
	...
	private:
		class Nodo {
			public:
				...
				void ih(Nodo * &ptrHoja, int llave, Valor* valor){
				...
				int insertar(Nodo * &base, int llave, Valor* valor){
					int exito = 0;
					int lado = IZQ;
					if(llave > this->llave){
						lado = DER;
				...
				void rdi(Nodo * &base,Nodo * a, Nodo * p, Nodo * h){
					//rotacion no necesaria, la unica que se ocupaba era la RDD
					base = h;
					a->hijo[DER] = h->hijo[IZQ];
					p->hijo[IZQ] = h->hijo[DER];
					h->hijo[IZQ] = a;
					h->hijo[DER] = p;
				}
	...
	public:
		...
		void insertar(int llave, Valor* valor){
			if(!raiz){
